...
filePath <- "C:/Users/ms/Documents/R/Pathway Projects/portfolio/Quantium Virtual Internship/Task 2/QVI_data.csv"
...
#### Calculating these measures over time for each store
#### Adding a new month ID column in the data with the format yyyymm.
# data[, YEARMONTH := ]
# Convert DATE column to month ID in yyyymm format
data[, YEARMONTH := format(DATE, "%Y%m")]

# Filter to stores that are present throughout the pre-trial period
pre_trial_stores <- data[, .N, by = .(STORE_NBR)][N == 12, STORE_NBR]
pre_trial_data <- data[STORE_NBR %in% pre_trial_stores]
...
#### Using the function created to calculate correlations 
...
#### Then, using the functions for calculating magnitude.
...
```{r}
#### Combining scores across the drivers by first merging our sales scores and customer scores into a single table
...
```{r}
#### Selecting control stores based on the highest matching store (closest to 1 but
#### not the store itself, i.e. the second ranked highest store)
#### Selecting the most appropriate control store for trial store 77 by finding the store with the highest final score.
...
#### Calculating the t-values for the trial months. After that, find the 95th percentile of the t distribution with the appropriate degrees of freedom
...
#### Computing a scaling factor to align control store customer counts to our trial store.
#### Then, appllying the scaling factor to control store customer counts.
#### Finally, calculating the percentage difference between scaled control store customers and trial customers.
...
#### Plot of everything in one nice graph.
...
We'll look at total sales first.
```{r}
#### Conducting visual checks on trends based on the drivers
measureOverTimeSales <- measureOverTime
 measureOverTimeSales[, YEARMONTH := as.numeric(YEARMONTH)]
...
ggplot(pastSales, aes(TransactionMonth, totSales, color = Store_type)) +
  ...
  labs(x = "Month of operation", y = "Total sales", title = "Average Monthly store sales over time")
```
